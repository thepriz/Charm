...
import svenhjol.meson.enums.VanillaOldChestMaterialType;
...
public class QuarkCompat implements IQuarkCompat {
    ...
    public boolean isInsideBigDungeon(ServerWorld world, BlockPos pos) {
        return WorldHelper.isPositionInsideStructure(world, pos, getBigDungeonStructure());
    }
    ...
    public String getBigDungeonResName() {
        return getBigDungeonStructure().getStructureName();
    }
    ...
    public Block getRandomChest(Random rand) {
        List<VanillaOldChestMaterialType> types = Arrays.asList(VanillaOldChestMaterialType.values());
        VanillaOldChestMaterialType type = types.get(rand.nextInt(types.size()));
