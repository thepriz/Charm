...
public class PacketHandler {
    ...
    public PacketHandler(MesonMod mod) {
        this(mod, "main", 1);
    }

    public PacketHandler(MesonMod mod, String channelName, int protocol) {
        this.mod = mod;
        String s = String.valueOf(protocol);

        this.channel = NetworkRegistry.ChannelBuilder
            .named(new ResourceLocation(mod.getId(), channelName))
    ...
    public <MSG> void register(Class<MSG> clazz, BiConsumer<MSG, PacketBuffer> encoder, Function<PacketBuffer, MSG> decoder, BiConsumer<MSG, Supplier<NetworkEvent.Context>> messageConsumer) {
        Meson.LOG.debug("Registering message " + clazz + ", index " + index);
     ...
     */
    public void sendToPlayer(IMesonMessage msg, ServerPlayerEntity player) {
